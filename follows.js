(function ( $ ){
	...
	*/
	...
	var defaultKey		= 'HrChKbhfuQILtjqugzd8BaQRiPhgSqjFaHzrUPdmuHL0oGd0', // Unique master Xively API key to be used as a default
		defaultFeeds	= [1282406454], // Comma separated array of Xively Feed ID numbers
		applicationName	= 'Kenzapp', // Replaces Xively logo in the header
			...
			xively.feed.history(id, {  duration: "6hours", interval: 30 }, function (data) {
				if(data.id == id) {
					...
					$('#feed-' + data.id + ' .duration-90').click(function() {
						$('#loadingData').foundation('reveal', 'open');
						updateFeeds(data.id, thisFeedDatastreams, '120days', 21600);
						return false;
					});
